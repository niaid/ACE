...
2. The rows represent genomic ranges of interest
...
The data from the 20 genes can be represented as a GRanges object
...
```{r}
femalecoreteam <- subset(se, colData(se)$sex == "Female")
```
...
The purpose of this exercise is to count the number of reads overlapping genes.  This is a typical first step in an RNA-seq
differential expression analysis.

Annotations (gene models) can come from different sources.  We’ll use a pre-packaged source here; a later exercise
retrieves  similar  information  from  a  GFF  file.  We will install the package first, then load it into memory.
